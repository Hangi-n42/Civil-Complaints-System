# Week3 vs Week4 A.X-4.0-Light 벤치마크 비교 리포트
...
Week3에서 벤치마크된 A.X-4.0-Light 모델이 Week4에서 다음 개선사항을 거친 후 성능 변화를 비교한다:
...
| answer_non_empty_rate | 0.0 | 0.0 | 1.0 | Repaired 기준 개선 |
| citation_match_rate | 0.0 | 0.0 | 1.0 | Repaired 기준 개선 |
| avg_latency_sec | 30.3459 | 13.0711 | 13.0711 | **-56.9%** ⬇️ |
| p95_latency_sec | 32.7393 | 14.5985 | 14.5985 | **-55.4%** ⬇️ |
...
### 4.2 Answer 품질 (Strict 정체, Repaired 개선)
- **Week3 원본**: 0.0 (비어있지 않은 답변 비율)
  - 해석: 100개 중 유효 answer가 없는 상태
- **Week4 Strict**: 0.0 (원본 기준 정체)
  - 의미: 모델 원본 출력만 보면 answer 생성 품질은 여전히 취약
- **Week4 Repaired**: 1.0 (자동 교정 후)
  - 의미: 자동 교정 정책 적용으로 모든 케이스에서 유효한 answer 확보
  - **Impact**: 0.0 → 1.0은 서비스 가용성 관점에서 극적 개선

### 4.3 Citation 정합성 (Strict 정체, Repaired 개선)
- **Week3 원본**: 0.0 (모든 케이스 정합성 실패)
  - 근거: citation 매핑/검증을 통과한 케이스 부재
- **Week4 Strict**: 0.0 (원본 기준 동일)
  - 의미: 원본 응답 자체는 citation 정합성 미흡
- **Week4 Repaired**: 1.0 (자동 교정 후)
...
|---|---:|---:|---:|---|
| road_safety | 7 | 1.0 | 1.0 | - Strict 한계, Repaired 정상화 |
| noise | 7 | 1.0 | 1.0 | - Strict 한계, Repaired 정상화 |
| flood | 7 | 1.0 | 1.0 | - Strict 한계, Repaired 정상화 |
| welfare | 7 | 1.0 | 1.0 | - Strict 한계, Repaired 정상화 |
| multi_request | 7 | 1.0 | 1.0 | - 복합요청도 Repaired 수렴 |
| waste | 7 | 1.0 | 1.0 | - Strict 한계, Repaired 정상화 |
| sinkhole | 7 | 1.0 | 1.0 | - Strict 한계, Repaired 정상화 |

**평가**: 모든 시나리오에서 Repaired 기준 100% 달성. 시나리오 편차보다 원본/보정 차이가 지배적.
...
1. **API 기준 파싱 로직 통합**
   ...
   - 효과: Strict에서의 회귀 없이 Repaired 가용성 확보
   - 해석: 정규화, 스키마 검증, 에러 핸들링 통일

2. **Answer 최소 품질 가드**
   - 공백/특수문자 정규화
   - 최소 길이 요구사항 적용
   - 효과: 빈 answer 케이스를 복원 가능한 구조로 전환
...
| 관점 | 변화 | 영향 |
|---|---|---|
| **가용성** | 0% → 100% | 서비스 제공 가능 케이스 전량 확보 |
| **신뢰성** | 0% → 100% | 모든 응답에 검증된 근거 첨부 가능 |
| **성능** | 30.3s → 13.1s | P95 응답시간 55% 단축 |
...
2. Strict 기준 answer_non_empty_rate = 0.0
   - 의미: 원본 출력은 여전히 전량 보정 의존 상태
   - 원인: 모델 자체 출력 형식/내용 안정성 한계
...
### Week3 결과
- [Stage 1 결과](../week3/ax4_stage1_ctx1024/model_benchmark_candidate_candidate_ax4_light.json)
- [Stage 1 요약](../week3/ax4_stage1_ctx1024/model_benchmark_candidate_candidate_ax4_light.md)
- [Stage 1 원본 리포트](../week3/WEEK3_BE3_AX4_AB_COMPARISON_REPORT.md)

### Week4 결과
- [Week4 결과](../../logs/evaluation/week4/ax4_ctx1024/model_benchmark_candidate_candidate_ax4_light.json)
- [Week4 요약](../../logs/evaluation/week4/ax4_ctx1024/model_benchmark_candidate_candidate_ax4_light.md)
- [Week4 raw responses](../../logs/evaluation/week4/ax4_ctx1024/raw_responses.jsonl)
- [Week4 parsed answers](../../logs/evaluation/week4/ax4_ctx1024/parsed_answers.jsonl)
