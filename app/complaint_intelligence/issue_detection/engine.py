...
class IssueDetectionEngine:
    ...
    _CONCRETE_TOPIC_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
        ...
        ("무단투기 반복", ("무단투기", "쓰레기", "적치", "방치", "폐기물", "청소")),
        ("공원 시설 파손", ("공원", "놀이터", "산책로", "벤치", "파손", "고장", "안전")),
        ("버스 노선/배차 불편", ("버스", "정류장", "노선", "배차", "도착 안내", "대중교통")),
        ("CCTV/방범 안전 요청", ("CCTV", "방범", "순찰", "사각지대", "야간 안전", "설치")),
        ("금연구역 흡연 단속 필요", ("금연구역", "흡연", "담배", "간접흡연", "담배꽁초")),
        ("불법 현수막 정비", ("불법 현수막", "현수막", "불법 광고물", "광고물", "시야", "미관")),
        ("반려동물 배설물/목줄 민원", ("반려동물", "배설물", "목줄", "유기동물", "개", "위생")),
    ...
    _TOPIC_KEYWORDS: dict[str, tuple[str, ...]] = {
        ...
        "쓰레기": ("쓰레기", "폐기물", "무단투기", "청소", "악취", "적치", "방치"),
        "주정차": ("주정차", "불법주정차", "주차", "단속", "차량"),
        "가로등": ("가로등", "보안등", "조명", "고장", "점멸"),
        "소음": ("소음", "진동", "공사", "층간"),
        "복지": ("복지", "지원", "급여", "생활비", "노인"),
        "배수": ("침수", "배수", "맨홀", "우수관", "빗물받이", "역류"),
        "공원시설": ("공원", "놀이터", "산책로", "벤치", "파손", "고장"),
        "교통서비스": ("버스", "정류장", "노선", "배차", "대중교통"),
        "방범": ("CCTV", "방범", "순찰", "사각지대", "야간 안전"),
        "흡연": ("금연구역", "흡연", "담배", "간접흡연", "담배꽁초"),
        "광고물": ("현수막", "광고물", "불법 광고물", "시야", "미관"),
        "반려동물": ("반려동물", "배설물", "목줄", "유기동물"),
        "인허가": ("인허가", "자격", "면허", "제출 서류", "기준"),
        "접근성": ("고령자", "장애인", "외국인", "접근성", "외국어"),
        "통학안전": ("어린이보호구역", "통학", "등하교", "학교 앞"),
    }
    _RISK_KEYWORDS = (
        "싱크홀", "침하", "꺼짐", "구멍", "위험", "파손", "균열", "누수", "침수", "화재", "사고", "움푹", "내려앉",
        "배수", "맨홀", "역류", "방범", "CCTV", "어린이보호구역", "통학", "야간 안전",
    )
    _UX_KEYWORDS = ("공공자전거", "자전거", "앱", "예약", "대여", "결제", "오류", "로그인")
    _ACCESSIBILITY_KEYWORDS = ("고령자", "장애인", "외국인", "디지털", "취약계층", "접근성", "어려움", "복잡", "글씨", "외국어", "휠체어")
    _REPEAT_KEYWORDS = ("재민원", "반복", "다시", "재발", "계속", "여러 번", "불만", "재문의", "재접수", "완료 안내", "처리 완료")
    ...
    def _operational_alerts(
        ...
        expanded_rules = (
            ...
            ("침수/배수 불량 위험", ("침수", "배수", "맨홀", "우수관", "하수도", "빗물받이", "역류")),
            ("무단투기 반복", ("무단투기", "쓰레기", "폐기물", "적치", "방치")),
            ("공원 시설 파손", ("공원", "놀이터", "산책로", "벤치", "파손", "고장")),
            ("가로등/보안등 고장", ("가로등", "보안등", "조명", "꺼짐", "고장")),
            ("버스 노선/배차 불편", ("버스", "정류장", "노선", "배차", "대중교통")),
            ("CCTV/방범 안전 요청", ("CCTV", "방범", "순찰", "사각지대", "야간 안전")),
            ("금연구역 흡연 단속 필요", ("금연구역", "흡연", "담배", "간접흡연", "담배꽁초")),
        ...
        for topic, keywords in expanded_rules:
            alerts.extend(
                self._keyword_operational_alerts(
                    recent_events,
                    reference_time,
    ...
    def _repeat_alerts(
        ...
        repeat_events = [
            ...
            or _contains_any(" ".join([str(event.reviewer_feedback or ""), str(event.feedback or "")]), self._REPEAT_KEYWORDS)
        ]
