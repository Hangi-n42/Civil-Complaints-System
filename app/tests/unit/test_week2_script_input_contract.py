...
def test_week2_delivery_sample_prefers_preprocessed_consulting_content():
    raw = {
    ...
    assert "담당 부서에 전달했습니다" not in record["raw_text"]


def test_week2_e2e_collect_raw_samples_supports_aihub_consulting_content(tmp_path, monkeypatch):
    ...
    assert "안내드리겠습니다" not in rows[0]["raw_text"]
